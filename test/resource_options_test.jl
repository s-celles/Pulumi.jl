...
@testitem "Resource option wire format" setup=[TestSupport] begin
    @testset "Identity and hierarchy" begin
        ...
        message = Pulumi._build_register_resource_request(TestSupport.base_request(
            parent = parent_urn,
    ...
    @testset "protect" begin
        @test Pulumi._build_register_resource_request(TestSupport.base_request(protect = true)).protect
        @test !Pulumi._build_register_resource_request(TestSupport.base_request()).protect
    end

    @testset "dependencies" begin
        urns = ["urn:pulumi:dev::proj::aws:s3/bucket:Bucket::a",
                "urn:pulumi:dev::proj::aws:s3/bucket:Bucket::b"]
        message = Pulumi._build_register_resource_request(TestSupport.base_request(dependencies = urns))
    ...
    @testset "retainOnDelete" begin
        @test Pulumi._build_register_resource_request(TestSupport.base_request(retainOnDelete = true)).retainOnDelete
        @test !Pulumi._build_register_resource_request(TestSupport.base_request()).retainOnDelete
    end
    ...
    @testset "deleteBeforeReplace" begin
        ...
        message = Pulumi._build_register_resource_request(TestSupport.base_request(deleteBeforeReplace = true))
        @test message.deleteBeforeReplace
        @test message.deleteBeforeReplaceDefined

        default = Pulumi._build_register_resource_request(TestSupport.base_request())
        @test !default.deleteBeforeReplace
        @test !default.deleteBeforeReplaceDefined
    end

    @testset "aliases" begin
        @testset "URN strings become TestSupport.Alias messages" begin
            urns = ["urn:pulumi:dev::proj::aws:s3/bucket:Bucket::old",
                    "urn:pulumi:dev::proj::aws:s3/bucket:Bucket::older"]
            message = Pulumi._build_register_resource_request(TestSupport.base_request(aliases = urns))

            @test length(message.aliases) == 2
            @test Base.all(a -> a isa TestSupport.Alias, message.aliases)  # `all` is ambiguous: Pulumi exports its own
        ...
        @testset "Alias messages are passed through" begin
            ...
            alias = TestSupport.Alias(Pulumi.PB.OneOf(:spec, spec))
            message = Pulumi._build_register_resource_request(TestSupport.base_request(aliases = [alias]))
    ...
    @testset "customTimeouts" begin
        @testset "All three timeouts" begin
            ...
            @test message.customTimeouts isa TestSupport.CustomTimeouts
        ...
        @testset "Absent timeouts stay unset" begin
            @test Pulumi._build_register_resource_request(TestSupport.base_request()).customTimeouts === nothing
            @test Pulumi._build_register_resource_request(
                TestSupport.base_request(customTimeouts = nothing)
            ).customTimeouts === nothing
        end
    end
end

@testitem "ResourceOptions plumbing" setup=[TestSupport] begin
